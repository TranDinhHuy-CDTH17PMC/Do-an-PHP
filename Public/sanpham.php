...
<body>
	...
	include_once "../Apps/Libs/helper.php";
	...
	<?php
	$db = new Database();
	$query_count = "select count(*) as counter from db_ghtshop.chitietsp where xoa = 1";
	$kq = $db->thuchien_query($query_count);
	$tongsanpham = $kq[0]['counter'];
	$tranghientai = input_get('trang');

	$link = tao_link(base_url('Public/sanpham'), array(
		'trang' => '{trang}'
	));
	$gioihan = 8;
	$phan_trang = phan_trang($link, $tongsanpham, $tranghientai, $gioihan);
	$query_select = "select * from db_ghtshop.chitietsp where xoa = 1 order by masp limit " . $phan_trang['batdau'] . "," . $phan_trang['gioihan'] . "";
